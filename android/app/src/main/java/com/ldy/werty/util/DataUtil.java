...
public final class DataUtil {
    ...
    public static synchronized void removeKey(Context context, String key) {
        setValuePrivate(context, key, null);
    }

    public static synchronized void removeKeyArray(Context context, String... keyArray) {
        removeValuePrivate(context, keyArray);
    }

    private static Object getValue(Context context, String key, Class<?> clazz, Object defValue) {
        Object ret = getValue(context, key, clazz);

        if (clazz == Integer.class) {
            if ((Integer) ret == -1) {
                ret = defValue;
            } else if (!isRealDB(ret)) {
                ret = -1;
            }
    ...
    private static Object getValue(Context context, String key, Class<?> clazz) {
        ...
        try {
            if (sqlLiteHelper == null) {
                sqlLiteHelper = new MySqlLiteHelper(context.getApplicationContext(), DB_NAME, null, DB_VERSION);
            ...
            if (cursor.getCount() > 0 && cursor.moveToFirst()) {
                if (clazz == Integer.class) {
                    ...
                    ret = !cursor.getString(0).equals("no");
                } else if (clazz == String.class) {
    ...
    private static boolean isRealDB(Object obj) {
        ...
        return !(obj.toString().equalsIgnoreCase("null") || obj.toString().equalsIgnoreCase("false"));
    }

    private static synchronized void setValuePrivate(Context context, String key, Object value) {
        if (sqlLiteHelper == null) {
            sqlLiteHelper = new MySqlLiteHelper(context.getApplicationContext(), DB_NAME, null, DB_VERSION);
        ...
        try {
            String sql = "select count(*) from " + TABLE_NAME + " where key = ?";
            if (value == null) {
    ...
    private static synchronized void removeValuePrivate(Context context, String... key) {
        if (sqlLiteHelper == null) {
            sqlLiteHelper = new MySqlLiteHelper(context.getApplicationContext(), DB_NAME, null, DB_VERSION);
            db = sqlLiteHelper.getWritableDatabase();
        }
        ...
        try {
            ...
            String sql = "delete from " + TABLE_NAME + " where key in(" + keyTmp + ")";
            db.execSQL(sql);
        } catch (Exception ex) {
            ex.printStackTrace();
        }
